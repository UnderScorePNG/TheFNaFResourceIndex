function closeNav() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

/*-----------------------------------------------------------------*/

function openNav1() {
    document.getElementById("sideNav1").style.width = "100%";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav2() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "100%";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav3() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "100%";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav4() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "100%";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav5() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "100%";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav6() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "100%";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav7() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "100%";
    document.getElementById("sideNav8").style.width = "0px";
}

function openNav8() {
    document.getElementById("sideNav1").style.width = "0px";
    document.getElementById("sideNav2").style.width = "0px";
    document.getElementById("sideNav3").style.width = "0px";
    document.getElementById("sideNav4").style.width = "0px";
    document.getElementById("sideNav5").style.width = "0px";
    document.getElementById("sideNav6").style.width = "0px";
    document.getElementById("sideNav7").style.width = "0px";
    document.getElementById("sideNav8").style.width = "100%";
}
